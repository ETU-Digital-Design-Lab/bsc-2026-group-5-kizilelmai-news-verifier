import csv, json, hashlib, os, re, numpy as np, pandas as pd
...
unused_embs = model.encode(['passage: ' + t for t in unused_texts], convert_to_numpy=True, show_progress_bar=False).astype(np.float32)
...
for i, row in enumerate(claims):
    text = row['text']
    claim_emb = model.encode(['query: ' + text], convert_to_numpy=True, show_progress_bar=False).astype(np.float32)[0]
    sims = (unused_embs @ claim_emb) / (np.linalg.norm(unused_embs, axis=1) * np.linalg.norm(claim_emb))
    best_idx = int(sims.argmax())
    best_item = unused_items[best_idx]
    
    gold = 'DOĞRU' if str(row['gold_label']) == '1' else 'YALAN'
