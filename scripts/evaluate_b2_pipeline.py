...
def main():
    ...
    record = manifest(__file__, inputs, {"seed": args.seed, "bootstrap_resamples": args.bootstrap_resamples,
                      "task": "B2 Frozen Evaluation (Sızıntısız Uçtan Uca) with fixed local corpus",
    ...
    try:
        ...
        with (out / "engine_stdout.log").open("w", encoding="utf-8") as log, contextlib.redirect_stdout(log):
            ...
            for start in range(0, len(rows), 16):
                ...
                vectors = engine.search_model.encode(["query: " + r["claim_text"] for r in batch], convert_to_numpy=True).astype(np.float32)
                ...
                for r, scores in zip(batch, sims):
                    ...
                    nearest.append({"claim_id": r["claim_id"], "record_id": corpus[idx]["id"], "cosine_similarity": float(scores[idx])})
